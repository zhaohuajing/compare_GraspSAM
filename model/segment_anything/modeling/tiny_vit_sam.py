...
# class VitAdapterTinyVit(TinyViT):
#     pass
...
class TinyViTReinAdapter(TinyViT):

    def __init__(
        self,
        distinct_cfgs=_tinyvit_cfgs,
        **kwargs,
    ):
        ...
        for cfgs in distinct_cfgs:
            ...
            self.reins.append(
                LoRAReins(
                    num_layers=reins_config["num_layers"],
                    embed_dims=reins_config["embed_dims"],
                    patch_size=16,
                    lora_dim=16,
                )
            )
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.layers[1:]):
            for idx_sublayer, sublayer in enumerate(layer.blocks):
                # print(i, idx_sublayer)
                # print(x.shape)
                x = sublayer(x)
                # print(x.shape)
                x = self.reins[i].forward(
                        x,
                        idx_sublayer,
                        batch_first=True,
                        has_cls_token=False,
...
class TinyViTMedicalAdapter(TinyViT):
    pass
