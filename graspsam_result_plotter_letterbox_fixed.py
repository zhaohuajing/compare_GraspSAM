...
def scale_intrinsics(args, width: int, height: int) -> np.ndarray:
    """
    Convert CameraInfo intrinsics to the current Jacquard-like image size.

    The current RGB-D converter does NOT stretch 1280x720/640x360 directly
    into 1024x1024. It pads the image to a square first, then resizes.
    Therefore the correct mapping is a uniform scale plus x/y padding.
    This matches the letterbox-aware conversion used in eval.py.
    """
    scale = min(float(width) / float(args.intr_w),
                float(height) / float(args.intr_h))
    pad_x = 0.5 * (float(width) - float(args.intr_w) * scale)
    pad_y = 0.5 * (float(height) - float(args.intr_h) * scale)
    ...
    print(
        f"[INFO] Letterbox intrinsics: scale={scale:.6f}, "
        f"pad_x={pad_x:.1f}, pad_y={pad_y:.1f}, "
        f"image_size=({width}, {height}), intr_size=({args.intr_w}, {args.intr_h})"
    )
    return K
