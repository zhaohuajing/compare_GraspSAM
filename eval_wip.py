...
def main(args, i=0):
    ...
    # run_id = time.strftime("%Y%m%d_%H%M%S") # denote folder name up to second
    run_id = time.strftime("%Y%m%d_%H") # denote folder name up to hour
    ...
    #--------------------------------------------
        ...
        #     # or ("color.png", "0.npy")
        # ]
        # test_dataset = JacquardDataset(root=rgbd_root,rgbd_pairs=rgbd_pairs,has_gt=False,
        #     output_size=1024, include_mask=True
        # )
    ...
    with torch.no_grad():
        ...
        for idx, data in enumerate(test_loader):
            ...
            targets = {}
            targets["masks"] = masks
            if args.dataset_name != "from_rgbd":
                targets["grasps"] = grasps    

        
            #--------------------------------------------------
            # Added for custom mask based image resizing: 
            #   alternative is to edit ./data/base_grasp_data.py with self.crop_size passed via JacquardDataset()
            # Below copied from the other eval.py with direct "mask_np = np.load(f"{rgbd_root}/im_label.npy")"; Needs edits to match with current Jac compatible form
            #--------------------------------------------------
            ...
            if args.seen_set and args.dataset_name != "from_rgbd":
...
if __name__ == "__main__":
    ...
    #-------------------------------------------------------
    # Added to choose (1) whether to crop theinput image and mask, and 
    #    (2) whether to use mask to additionally remove background
    #-------------------------------------------------------
