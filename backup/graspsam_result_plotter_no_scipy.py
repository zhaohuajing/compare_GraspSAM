...
def quat_to_matrix(quat) -> np.ndarray:
    ...
    This local implementation avoids scipy.spatial.transform so the plotter
    does not depend on a user-site SciPy/NumPy version pairing.
    """
...
def matrix_to_quat(Rm: np.ndarray) -> np.ndarray:
    """
